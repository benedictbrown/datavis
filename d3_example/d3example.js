...
// web browsers will only load external files over the network, not local files, so you have to start up a web server
...
d3.text('d3example.csv') // load d3example.csv as a text file
  .then((data) => d3.csvParseRows(data, d3.autoType)) // parse the CSV data into a 2D array, without assuming a header row
  .then((data) => {                                   // d3.autoType is new in d3 v5, and converts numbers instead of leaving them strings
    sampleData = data;
    updatePlot();
  });
...
function updatePlot() {
  ...
  const xScale = d3.scaleLinear().domain([ 0, sampleData[index].length + 1 ]).range([ padding, w - padding ]);
  ...
  const yScale = d3.scaleLinear().domain([ d3.min(sampleData[index]) - 1, d3.max(sampleData[index]) + 1]).range([ h - padding, padding ]);

  const plot_circles = plot.selectAll('circle');    // selector for all circles in the plot
  const plot_data = plot_circles.data(sampleData[index]); // bind the latest version of sampleData[index]
  
  const plot_new_data = plot_data.enter() // do the following for every element of sampleData[index] that is getting added
